...
object HdfsUtils {
  ...
  def listPath(hdfs: FileSystem, path: String): List[String] = {
    val listBuffer = ListBuffer[String]()
    val filesStatus = hdfs.listStatus(new Path(path), new MyPathFilter)
    filesStatus.foreach { status =>
      println(status.getPath.toString)
      listBuffer += status.getPath.toString
    }
    listBuffer.toList
  }
...
//  }
  ...
  def readGZFile(fileSystem : FileSystem, hdfsFilePath: String): BufferedReader = {
    val conf = new Configuration()
    val factory = new CompressionCodecFactory(conf)

    try {
      val fsis = fileSystem.open(new Path(hdfsFilePath))
      val codec = factory.getCodec(new Path(hdfsFilePath))
      val reader = if(null == codec) {
        new BufferedReader(new InputStreamReader(fsis))
      } else {
        new BufferedReader(new InputStreamReader(codec.createInputStream(fsis)))
      }
      reader
    } catch {
      case e: Exception => {
